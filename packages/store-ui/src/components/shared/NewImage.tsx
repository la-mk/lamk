...
import { hooks } from '@sradevski/blocks-ui';
...
const getSrcSet = (
  imageId: string,
  imageBucket: string,
  getFullPath: ImageProps['getFullPath'],
  parameters: any,
) => {
  return [1, 2, 3]
    .map(
      dpr =>
        `${getFullPath(imageId, imageBucket, { ...parameters, dpr })} ${dpr}x`,
    )
    .join(',\n');
};
...
export const NewImage = ({
  ...
  if (fetchFailed || !imageId || !imageBucket) {
  ...
  return (
    <picture onError={() => setFetchFailed(true)}>
      <source
        type='image/webp'
        srcSet={getSrcSet(imageId, imageBucket, getFullPath, {
          format: 'webp',
          ...params,
        })}
      />
      <source srcSet={getSrcSet(imageId, imageBucket, getFullPath, params)} />
      <SizedImage
        {...otherProps}
        height={params.h}
        width={params.w}
        ...
        loading='lazy'
        src={getFullPath(imageId, imageBucket, params)}
        alt={alt}
      />
    </picture>
  );
