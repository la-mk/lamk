...
const parsePriceFilter = (
  ...
  const fromPrice = toNumber(
    filtering.minCalculatedPrice.$gt || filtering.minCalculatedPrice.$gte || min
  );
  const toPrice = toNumber(
    filtering.minCalculatedPrice.$lt || filtering.minCalculatedPrice.$lte || max
  );
...
const PredefinedRanges = ({
  ...
  onRangeSelected: (range: [number, number]) => void;
  t: TFunction;
}) => {
  return (
    <Flex direction="column" align="flex-start">
      ...
      <Button onClick={() => onRangeSelected([100, 1000])} variant="ghost">
        <Text color={start === 100 && end === 1000 ? "primary" : "text.dark"}>
          <>
            100 {t(`currencies.${currency}`)} - 1000{" "}
...
export const PriceFilter = ({
  ...
  const handleChangeDone = (f: any, t: any) => {
    onChange({
      ...filters,
      filtering: {
        ...(filters.filtering || {}),
    ...
    <CustomCard {...props}>
      ...
      <Flex mt={3} align="center">
        <Input
          type="number"
          onChange={(_e, val) =>
            (val as number) <= end && setRange([val as number, end])
          }
        ...
        <Input
          type="number"
          onChange={(_e, val) =>
            (val as number) >= start && setRange([start, val as number])
          }
          onBlur={() => handleChangeDone(start, end)}
          max={max}
