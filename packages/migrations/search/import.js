...
const sdk = require('@sradevski/la-sdk');
...
const importToSearch = async (products) => {
  ...
  await sdk.sdk.request('search').create({model: 'products', item: products[0]});
...
const main = async () => {
  ...
  const buffer = [];
  await dbclient.collection('products').find().forEach((product) => {
    buffer.push(product);
  });
