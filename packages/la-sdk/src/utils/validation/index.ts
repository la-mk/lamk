...
const filterRequired = (errors: DefinedError[]) => errors.filter(error => error.keyword !== 'required')

const transformToErrorObject = (errors: DefinedError[]): ValidationErrorResponse => {
...
export const validateSingle = <T extends any>(schema: JSONSchemaType<T>, val: any, selector: string) => {
...
export const validate = <T extends any>(schema: JSONSchemaType<T>, data: T | Partial<T>, ignoreRequired: boolean = false) => {
  ...
  const errors = ignoreRequired ? filterRequired(validate.errors as DefinedError[]) : validate.errors as DefinedError[];
